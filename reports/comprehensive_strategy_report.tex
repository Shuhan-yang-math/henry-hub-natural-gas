...
\fancyhead[R]{Research Report -- August 11, 2026}
\fancyfoot[C]{\thepage}
...
Selected research version: & \code{eia930\_central40\_florida60\_10pct\_with\_event\_veto}\\
Sample: & July 3, 2017--July 13, 2026; 2,269 trading days\\
...
\begin{table}[h]
\centering
\caption{Formal full-sample performance}
\begin{tabular}{lrr}
\toprule
...
The selected EIA-930 version can be compared on a common 1,737-day overlap
from July 25, 2019 through July 13, 2026. The matched baseline already includes
the event veto, so the difference below isolates the fixed 10\% EIA-930 sleeve.

\begin{table}[h]
\centering
\caption{Selected EIA-930 common-overlap performance}
\begin{tabular}{lrrr}
\toprule
Metric & Matched baseline & Central 100\% & Central 40\% / Florida 60\%\\
\midrule
Net Sharpe & 1.875 & 1.992 & \textbf{2.115}\\
Net Sortino & 3.195 & 3.329 & \textbf{3.643}\\
Net CAGR & 18.03\% & 19.36\% & \textbf{19.38\%}\\
Maximum drawdown & -5.59\% & -6.07\% & \textbf{-5.27\%}\\
Daily win rate & 53.02\% & 53.66\% & \textbf{54.06\%}\\
Total net return & 217.35\% & 243.11\% & \textbf{243.70\%}\\
\bottomrule
\end{tabular}
\end{table}

Relative to the previous Central sleeve, the selected blend raises Sharpe by
0.122 and Sortino by 0.314 while reducing maximum drawdown by 0.80 percentage
points. Its simple sum of daily incremental net returns is -0.13 percentage
points, so the gain is mainly downside diversification rather than higher
unconditional return. The approved 2017-start formal artifact remains
unchanged and is reported separately because its history is longer.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../results/experiments/eia930_selected/latest_strategy_dashboard.png}
\caption{Selected EIA-930 strategy dashboard.}
\end{figure}
...
a pure BSEE/Sabine short veto. The veto can cancel a conflicting short but
cannot create or amplify a position. The nine internal fundamental signals
remain unchanged.
...
locations and forecast days 1--5. Wind is adjusted to hub height with
\begin{equation}
...
The selected EIA-930 version also funds a fixed 10\% sleeve from fundamentals.
...
selected EIA overlap contains seven actual veto dates.
...
2025 is the only full losing calendar year, with a -3.60\% return and -0.533
Sharpe. The main weakness was delayed reaction to several sharp reversals,
rather than continuous small transaction-cost losses.

On the selected EIA-930 common-overlap calculation, the 2025 Sharpe improves
to -0.076. The year remains negative, but the regional multi-fuel power signal
reduces part of the original loss.
...
\begin{itemize}
  ...
  \item Selected EIA-930 notebook:
        \path{notebooks/06_eia930_central_florida_40_60.ipynb}
  ...
  \item Selected evaluator:
        \path{naturalgas/evaluate_eia930_selected_enhancement.py}
  \item Selected summary:
        \path{results/experiments/eia930_selected/summary.json}
  \item Selected dashboard:
        \path{results/experiments/eia930_selected/latest_strategy_dashboard.png}
