...
Historical formal sample: & July 3, 2017--July 13, 2026; 2,264 trading days\\
Selected common sample: & July 25, 2019--July 13, 2026; 1,748 trading days\\
...
Sharpe, zero risk-free rate & \textbf{1.667} & \textbf{1.718}\\
...
direction guard, so its 1.667 Sharpe is not the current selected result.

The selected D1--3 version can be compared on a common 1,748-day overlap from
...
\midrule
Net Sharpe & 2.119 & 2.181 & \textbf{2.228}\\
Net Sortino & 3.663 & 3.787 & \textbf{3.881}\\
Net CAGR & \textbf{19.20\%} & 18.74\% & 19.05\%\\
Maximum drawdown & -5.30\% & -4.51\% & \textbf{-4.16\%}\\
Daily win rate & \textbf{54.06\%} & 53.78\% & 52.35\%\\
Total net return & \textbf{240.11\%} & 231.09\% & 237.22\%\\
\bottomrule
\end{tabular}
\end{table}
...
$\operatorname{mean}(g_t)/\operatorname{sample\_std}(g_t)\sqrt{252}$.
Sortino divides annualized mean log return by
$\sqrt{\operatorname{mean}(\min(g_t,0)^2)}\sqrt{252}$. This is a zero-target,
...
Relative to current D1--5, the selected version raises Sharpe by 0.108 and
Sortino by 0.218 while reducing maximum drawdown by 1.14 percentage points.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../results/models/v03_d1_3_storage_guard/dashboard.png}
...
Weekly EIA storage for a week ending Friday becomes usable at
the actual official WNGSR publication timestamp (normally Thursday at 10:30
a.m. Eastern, with audited holiday exceptions). Monthly EIA reference month M
becomes usable at the start of M+3 in
...
is set to zero. The HDD-revision branch is active in January--May and
September--December, disabled in June--August, and has no CDD substitute. The
guard cannot create or amplify exposure.
...
Florida is constructed from the Florida balancing authorities that are
complete on each source gas day. A partial day is aggregated from the
available respondents and standardized against the same continuous history,
which usually contains all nine BAs; it then remains in that history for
future rolling means and standard deviations. This removes the legacy
cross-region completeness coupling and retains all five previously omitted
Florida-outage return dates.
...
\midrule
Development, 2017--2020 & 1.693 & 12.41\% & -4.39\%\\
Validation, 2021--2023 & 1.880 & 18.66\% & -5.15\%\\
First-look holdout, 2024+ & 1.415 & 13.05\% & -6.14\%\\
Full sample & \textbf{1.667} & \textbf{14.59\%} & \textbf{-6.14\%}\\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Current D1--5 & \textbf{2.785} & 2.145 & 1.718 & 2.119 & \textbf{19.20\%} & -5.30\%\\
D1--3, no guard & 2.757 & 2.232 & 1.779 & 2.181 & 18.74\% & -4.51\%\\
\textbf{Selected D1--3 + amplifier} & 2.782 & \textbf{2.279} & \textbf{1.835} & \textbf{2.228} & 19.05\% & \textbf{-4.16\%}\\
\bottomrule
\end{tabular}
\end{table}

The D1--3 horizon provides most of the drawdown improvement. Adding the
storage amplifier raises full Sharpe from 2.181 to 2.228, validation Sharpe
from 2.232 to 2.279, and first-look Sharpe from 1.779 to 1.835. The selected
...
\midrule
2019 partial & 7.83\% & 2.710 & 2023 & 12.00\% & 1.501\\
2020 & 26.57\% & 2.830 & 2024 & 27.27\% & 3.265\\
2021 & 10.66\% & 1.792 & 2025 & 1.63\% & 0.234\\
2022 & 41.43\% & 3.270 & 2026 partial & 8.98\% & 1.981\\
\bottomrule
\end{tabular}
\end{table}
...
The historical formal artifact loses 3.55\% with a -0.525 Sharpe in 2025. On
the current common-sample calculation, D1--5 loses 0.43\% with a -0.060 Sharpe,
unguarded D1--3 earns 1.08\% with a 0.154 Sharpe, and the selected
storage-amplified D1--3 version earns 1.63\% with a 0.234 Sharpe. Its 2025
maximum drawdown is -3.94\%, versus -4.57\% for D1--5. The latest version
...
\begin{itemize}
  \item Historical baseline notebook:
        \path{notebooks/01_model_v01_south_central_storage.ipynb}
  \item Selected notebook:
        \path{notebooks/07_model_v03_d1_3_storage_guard.ipynb}
  \item Evaluator: \path{naturalgas/evaluate_model_v01_south_central_storage.py}
  \item Data manifest: \path{DATA_MANIFEST.md}
  \item Formal summary: \path{results/models/v01_south_central_storage/summary.json}
  \item Weights: \path{results/models/v01_south_central_storage/strategy_weights.csv}
  \item Period results: \path{results/models/v01_south_central_storage/period_comparison.csv}
  \item Annual results: \path{results/models/v01_south_central_storage/annual_comparison.csv}
  \item Selected evaluator:
        \path{naturalgas/evaluate_model_v03_d1_3_storage_guard.py}
  \item Selected summary:
        \path{results/models/v03_d1_3_storage_guard/summary.json}
  \item Selected dashboard:
        \path{results/models/v03_d1_3_storage_guard/dashboard.png}
